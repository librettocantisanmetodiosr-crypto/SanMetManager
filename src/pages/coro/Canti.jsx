...
export default function Canti() {
  ...
  const salva = async () => {
    ...
    setSaving(true)
    if (modal === 'nuovo') {
      const { error } = await supabase.from('canti').insert({ ...form, autore_id: profilo?.id })
  ...
  // Formatta testo con accordi in colore diverso
  const formattaTesto = (testo, accordi) => {
    ...
    return righe.map((riga, i) => (
      <div key={i} style={{ marginBottom: 2 }}>
        {riga.split(/(\[[^\]]+\])/g).map((parte, j) =>
          parte.startsWith('[') ? (
            <span key={j} style={{ color: 'var(--blue)', fontWeight: 800, fontSize: '0.85rem' }}>{parte.slice(1,-1)}</span>
          ) : (
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div className="flex items-center justify-between mb-4">
        <h1>🎵 Canti</h1>
        {isResponsabile && <button className="btn btn-primary btn-sm" onClick={() => { setForm({ titolo:'',categoria:'',tonalita:'',testo:'',accordi:'' }); setModal('nuovo') }}>＋ Aggiungi</button>}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
        <input className="form-control" placeholder="🔍 Cerca canto…" value={cerca} onChange={e => setCerca(e.target.value)} style={{ flex: 1 }} />
        <select className="form-control" value={filtroCategoria} onChange={e => setFiltroCategoria(e.target.value)} style={{ width: 130 }}>
                ...
                <div className="flex items-center justify-between">
                  <div style={{ flex: 1, cursor: 'pointer' }} onClick={() => setVistaModal(c)}>
                    ...
                    <div style={{ fontSize: '0.75rem', color: 'var(--gray-500)', marginTop: 2 }}>
                      {c.categoria && <span className="badge badge-green" style={{ marginRight: 6 }}>{c.categoria}</span>}
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    {isResponsabile && (
                      <button
                        className="btn btn-sm"
                        style={{
                          background: cantoAttivo === c.id ? 'var(--red)' : 'var(--primary)',
                          color: '#fff', fontWeight: 800, minWidth: 70
                        }}
                        onClick={() => lancia(c.id)}
                      >
                    ...
                    {isResponsabile && (
                      <>
                        <button className="btn btn-outline btn-sm btn-icon" onClick={() => { setForm({ titolo:c.titolo, categoria:c.categoria||'', tonalita:c.tonalita||'', testo:c.testo||'', accordi:c.accordi||'' }); setModal(c) }}>✏️</button>
      ...
      {vistaModal && (
        <div className="modal-overlay" onClick={() => setVistaModal(null)}>
          <div className="modal" style={{ borderRadius: '20px 20px 0 0', maxHeight: '85vh' }} onClick={e => e.stopPropagation()}>
            ...
            {!vistaModal.testo && <p className="text-muted text-sm">Nessun testo inserito per questo canto.</p>}
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <input className="form-control" value={form.titolo} onChange={e => setForm(f => ({ ...f, titolo: e.target.value }))} />
            </div>
            <div className="form-row">
              <div className="form-group">
                <label className="form-label">Categoria</label>
                <input className="form-control" value={form.categoria} onChange={e => setForm(f => ({ ...f, categoria: e.target.value }))} placeholder="Es: Offertorio" />
              </div>
              <div className="form-group">
                <label className="form-label">Tonalità</label>
                <input className="form-control" value={form.tonalita} onChange={e => setForm(f => ({ ...f, tonalita: e.target.value }))} placeholder="Es: Sol maggiore" />
              </div>
            </div>
            <div className="form-group">
              <label className="form-label">Testo (usa [Accordo] per gli accordi)</label>
              <textarea className="form-control" rows={6} value={form.testo} onChange={e => setForm(f => ({ ...f, testo: e.target.value }))} placeholder="[Sol]Tu sei la mia vi[Re]ta..." />
            </div>
